...
export function updateProduct(
  brandId: string,
  productId: number,
  updates: Partial<{ name: string; category: string; price: string; stock: number; description: string; image: string }>
): boolean {
  const brands = readBrands();
...
/** Called after a successful checkout to reduce inventory for what was
 * bought. Floors at 0 — never goes negative. */
export function decrementStock(brandId: string, productId: number, quantity: number): boolean {
  ...
  const current = brands[brandIdx].products[productIdx].stock;
  brands[brandIdx].products[productIdx].stock = Math.max(0, current - quantity);
