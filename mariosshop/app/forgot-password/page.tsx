...
export default function ForgotPasswordPage() {
  ...
  // Only shown as a fallback if the real email send fails (e.g. Resend
  // misconfigured) — normally the code just arrives in the user's inbox.
  ...
  const handleRequestCode = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    alert('DEBUG result: ' + JSON.stringify(result)); // TEMP — remove after debugging
        ...
        <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-6 md:p-8 backdrop-blur-md">
          ...
          {devCode && step === 'code' && (
            <div role="status" className="mb-4 rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-xs font-semibold text-amber-300">
              📧 Couldn't send the email, so here's your code for now: <span className="font-mono text-sm">{devCode}</span>
            </div>
          )}
