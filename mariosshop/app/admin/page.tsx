        ...
        <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-[#09090b]/95 px-3 py-3 backdrop-blur-xl lg:hidden">
          <div className="flex items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-2.5">
              <div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-red-600 font-black">M</div>
              <div className="min-w-0"><p className="truncate text-xs font-black">MARIOS SHOP</p><p className="text-[9px] uppercase tracking-widest text-zinc-600">Admin Panel</p></div>
            </div>
            <button onClick={() => router.push('/')} className="shrink-0 rounded-lg border border-white/[0.08] px-3 py-2 text-[10px] font-bold text-zinc-400">Store ↗</button>
          </div>
          <div className="mt-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-2">
            <p className="px-2 pb-2 text-[9px] font-black uppercase tracking-[0.2em] text-zinc-600">Admin Categories</p>
            <div className="grid grid-cols-2 gap-1.5">
              {navItems.map((item) => (
                <button key={item.id} onClick={() => go(item.id)} className={`flex min-h-11 items-center gap-2 rounded-lg px-3 py-2 text-left text-[11px] font-black transition ${activeSection === item.id ? 'bg-red-600 text-white' : 'bg-white/[0.04] text-zinc-400 active:bg-white/[0.08]'}`}>
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-black/20 text-xs">{item.icon}</span>
                  <span className="min-w-0 flex-1 truncate">{item.label}</span>
                  {item.badge !== undefined && item.badge > 0 && <span className="rounded-full bg-black/20 px-1.5 py-0.5 text-[9px]">{item.badge}</span>}
                </button>
              ))}
        ...
        <main className="min-h-screen lg:ml-[250px]"><div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-9">
          <div className="mb-5 flex flex-col gap-3 sm:mb-7 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="text-2xl font-black tracking-tight sm:text-3xl">{navItems.find((x) => x.id === activeSection)?.label}</h1>
              <p className="mt-1 text-xs text-zinc-600">{navItems.find((x) => x.id === activeSection)?.desc}</p>
            </div>
            <div className="flex items-center gap-2">
              {activeSection !== 'overview' && <button onClick={() => go('overview')} className="rounded-xl bg-red-600 px-4 py-2.5 text-xs font-black text-white shadow-lg shadow-red-600/10">← Dashboard Home</button>}
              <button onClick={() => window.location.reload()} className="hidden rounded-xl bg-white/[0.06] px-4 py-2.5 text-xs font-bold text-zinc-300 hover:bg-white/[0.1] transition sm:block">Refresh</button>
              <button onClick={() => router.push('/')} className="hidden rounded-xl border border-white/[0.07] bg-white/[0.025] px-4 py-2.5 text-xs font-bold text-zinc-400 hover:text-white transition sm:block">View Store ↗</button>
            </div>
          </div>
