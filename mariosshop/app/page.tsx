...
export default function HomePage() {
  ...
  // sees the same list, and it refreshes automatically when anyone submits
  // a new one, thanks to the realtime subscription.
  useEffect(() => {
    const refreshReviews = () => {
      getHomeReviews()
        .then(setReviews)
        .catch((error: unknown) => console.error('Unable to load reviews:', error));
    };
    refreshReviews();
    const unsubscribeReviews = onHomeReviewsChanged(refreshReviews);
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const saved = await addHomeReview({
        user: userName,
        item: itemPurchased,
        rating,
        comment,
      ...
      // Refresh from the shared source instead of trusting only our own
      // optimistic copy, so we're showing exactly what's actually saved.
      getHomeReviews()
        .then(setReviews)
        .catch((error: unknown) => console.error('Unable to refresh reviews:', error));
      ...
      setReviewSubmitError(
        error instanceof Error ? error.message : 'Something went wrong submitting your review. Please try again.'
      );
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-lg rounded-3xl p-6 md:p-8 shadow-2xl relative">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-4">
              ...
              {reviewSubmitError && (
                <p className="text-xs text-red-400">{reviewSubmitError}</p>
              )}
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={isSubmittingReview} className="flex-1 py-3 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-bold rounded-xl text-sm transition shadow-lg shadow-red-600/20">
                  {isSubmittingReview ? '…' : t('submit_review')}
                </button>
              </div>
            </form>
          </div>
